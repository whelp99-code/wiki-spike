...
Migrate one source at a time into final-workspace, non-serving cohorts. Each cohort must complete a source-specific 14-day shadow period before cutover. Prior systems remain read-only for 90 days after that cohort's activation; they are migration inputs, never peer serving gateways or query-time fallback.
