# ADR-0022: ActionIntent and WriteProposal Boundary

- Status: Accepted for P4-11
- Date: 2026-07-23
- Scope: `action_intent.py + write_proposal.py`

## Context

Phase 4 Runtime must remain deterministic at orchestration and policy boundaries even when retrieval or model components are probabilistic. ActionIntent and WriteProposal Boundary is isolated from Storage, credentials, connectors, UI, and external action execution.

## Decision

Implement explicit confirmation, no execution, Core command port, proposal dedupe. All public objects are versioned, canonical, content-bound, and workspace/generation aware. Unknown data fails closed. Source or model bodies stay behind references except where a user-facing answer contract explicitly needs presentation text.
...
The change is additive. Reverting P4-11 removes its module, tests, schema/document evidence, and leaves prior Runtime and signed Phase 3 artifacts unchanged.
