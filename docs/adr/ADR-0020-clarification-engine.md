# ADR-0020: Bounded Clarification Engine

- Status: Accepted for P4-09
- Date: 2026-07-23
- Scope: `clarification.py`

## Context

Phase 4 Runtime must remain deterministic at orchestration and policy boundaries even when retrieval or model components are probabilistic. Bounded Clarification Engine is isolated from Storage, credentials, connectors, UI, and external action execution.

## Decision

Implement information gain, dedupe, operation budget, expiry, safe default. All public objects are versioned, canonical, content-bound, and workspace/generation aware. Unknown data fails closed. Source or model bodies stay behind references except where a user-facing answer contract explicitly needs presentation text.
...
The change is additive. Reverting P4-09 removes its module, tests, schema/document evidence, and leaves prior Runtime and signed Phase 3 artifacts unchanged.
