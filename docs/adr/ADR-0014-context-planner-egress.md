# ADR-0014: Context Planner와 Egress Policy

- Status: Accepted for P4-03
- Date: 2026-07-23
- Scope: `context.py`

## Context

Phase 4 Runtime must remain deterministic at orchestration and policy boundaries even when retrieval or model components are probabilistic. Context Planner와 Egress Policy is isolated from Storage, credentials, connectors, UI, and external action execution.

## Decision

Implement field minimization, sensitivity lattice, local-only secret routing, token cap. All public objects are versioned, canonical, content-bound, and workspace/generation aware. Unknown data fails closed. Source or model bodies stay behind references except where a user-facing answer contract explicitly needs presentation text.
...
The change is additive. Reverting P4-03 removes its module, tests, schema/document evidence, and leaves prior Runtime and signed Phase 3 artifacts unchanged.
