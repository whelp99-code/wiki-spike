...
usage() {
  printf '%s\n' \
    "Usage: $0 <unsigned-profile.json> <authorized_at> <approver-envelope.json> <owner-envelope.json> <assembled.json>" \
    "Assembles public Mac persistence receipt envelopes." \
    "Pass exactly two envelopes, approver then owner." \
    "Writes a create-only assembled receipt."
}
...
if path_has_symlink "$profile" || path_has_symlink "$approver_envelope" || path_has_symlink "$owner_envelope"; then
  printf 'persistence receipt assemble refused: input would be a symlink\n' >&2
  exit 2
fi

if [[ ! -f "$profile" ]]; then
  printf 'unsigned profile is missing\n' >&2
  exit 2
fi
...
refuse_symlink_output() {
  printf 'persistence receipt assemble refused: output would be a symlink\n' >&2
  exit 2
}
...
if [[ -e "$dest" ]]; then
  printf 'persistence receipt assemble refused: output already exists\n' >&2
  exit 2
fi
...
if ! first_role="$(envelope_role "$approver_envelope" 2>/dev/null)"; then
  printf 'persistence receipt assemble refused: envelopes must be ordered approver then owner\n' >&2
  exit 2
fi
...
printf 'Wrote assembled receipt to %s\n' "$dest"
