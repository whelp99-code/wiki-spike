...
class RecallServeSnapshotV2:
    ...
    def __post_init__(self) -> None:
        ...
        if any(len(item) != 2 or not all(isinstance(ref, str) and ref for ref in item) for item in markers) or tuple(sorted(set(markers))) != markers:
            raise InvalidContractValue("unverified conflicts must be sorted and unique reference pairs")
        ...
        if any(near not in displayed or far in displayed for near, far in self.unverified_conflicts):
            raise InvalidContractValue("unverified conflict markers must bind a displayed winner to a withheld candidate")
    ...
    def to_mapping(self) -> dict[str, Any]:
        result = {k: getattr(self, k) for k in self.FIELDS}; result.update({"provenance_component_labels": list(self.provenance_component_labels), "provenance_component_states": [x.to_mapping() for x in self.provenance_component_states], "authorization": self.authorization.to_mapping(), "candidates": [x.to_mapping() for x in self.candidates], "conflicts": [x.to_mapping() for x in self.conflicts], "citations": [x.to_mapping() for x in self.citations], "unverified_conflicts": [list(item) for item in self.unverified_conflicts], "continuation": None if self.continuation is None else self.continuation.to_mapping()}); result.update({n: getattr(self, n).to_mapping() for n in ("global_floor", "binding", "recovery", "route", "cohort", "deletion", "consent")}); return result
...
def make_recall_snapshot_v2(body: Mapping[str, Any], *, seal_continuation: Callable[[Mapping[str, Any]], "RecallContinuationV2"] | None = None) -> RecallServeSnapshotV2:
    ...
    """
    body = dict(body)
    body.setdefault("unverified_conflicts", [])
    ...
    values["selected_conflicts_digest"] = canonical_ledger_digest("snapshot-selected-conflicts-v2", {"conflicts": values["conflicts"], "unverified_conflicts": values["unverified_conflicts"]})
